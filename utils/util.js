...
function get_new_coords(init_loc, distance, bearingDegs) {
    ...
    */
    ...
    var new_lat = Math.asin(Math.sin(init_coords[0]) * Math.cos(distance / R) + Math.cos(init_coords[0]) * Math.sin(distance / R) * Math.cos(bearing));
    var new_lon = init_coords[1] + Math.atan2(Math.sin(bearing) * Math.sin(distance / R) * Math.cos(init_coords[0]), Math.cos(distance / R) - Math.sin(init_coords[0]) * Math.sin(new_lat));
